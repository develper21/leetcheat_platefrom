...
export default function EnhancedFilters({ filters, onFiltersChange, onReset }: EnhancedFiltersProps) {
  ...
  const filterSections = [
    { title: 'Category', sectionKey: 'category' as keyof FilterState, items: categories, icon: Target },
    { title: 'Tags', sectionKey: 'tags' as keyof FilterState, items: tags, icon: Star },
    { title: 'Companies', sectionKey: 'companies' as keyof FilterState, items: companies, icon: Star },
    { title: 'Status', sectionKey: 'status' as keyof FilterState, items: statusOptions, icon: Star },
    { title: 'Time Complexity', sectionKey: 'timeComplexity' as keyof FilterState, items: timeComplexities, icon: Clock },
    { title: 'Space Complexity', sectionKey: 'spaceComplexity' as keyof FilterState, items: spaceComplexities, icon: Target },
  ];
        ...
        {isExpanded && (
          <div className="px-3 pb-3 space-y-2">
            {displayItems.map((item, index) => (
              <label key={`${item}-${index}-${sectionKey}-${item.replace(/\s+/g, '-').toLowerCase()}`} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 p-1 rounded">
                <input
                  type="checkbox"
    ...
    <Card className="w-full">
      ...
      <CardContent className="p-0 space-y-0">
        <div className="max-h-96 overflow-y-auto">
          ...
          {filterSections.map(({ title, sectionKey, items, icon: Icon }) => (
            <FilterSection
              key={sectionKey}
              title={title}
              ...
              icon={Icon}
              maxItems={sectionKey === 'category' ? 6 : 4}
            />
          ))}
        </div>
      </CardContent>
