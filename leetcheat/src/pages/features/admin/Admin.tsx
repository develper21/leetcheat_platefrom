...
export default function Admin({ currentUser, onBack }: AdminProps) {
  ...
  const [newQuestion, setNewQuestion] = useState<Omit<TheoryQuestion, 'id' | 'tags' | 'companies'> & { tags: string; companies: string }>({
    title: '',
    slug: '',
    difficulty: 'Easy',
    category: '',
  ...
  const loadQuestions = async () => {
    try {
      const questionsData = await MockQuestionAPI.getAllQuestions();
  ...
  const handleCreateQuestion = async () => {
    try {
      const questionData: Omit<TheoryQuestion, 'id'> = {
        ...
        tags: newQuestion.tags ? newQuestion.tags.split(',').map(t => t.trim()).filter(t => t) : [],
        companies: newQuestion.companies ? newQuestion.companies.split(',').map(c => c.trim()).filter(c => c) : []
  ...
  const handleUpdateQuestion = async () => {
    ...
    try {
      const updatedQuestion: TheoryQuestion = {
        ...editingQuestion,
        tags: editingQuestion.tags ? editingQuestion.tags.split(',').map(t => t.trim()).filter(t => t) : [],
        companies: editingQuestion.companies ? editingQuestion.companies.split(',').map(c => c.trim()).filter(c => c) : []
      };

      await MockQuestionAPI.updateQuestion(updatedQuestion.id, updatedQuestion);
        ...
        <Dialog open={!!editingQuestion} onOpenChange={() => setEditingQuestion(null)}>
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                <Label htmlFor="edit-companies">Companies (comma-separated)</Label>
                <Input
                  id="edit-companies"
                  value={editingQuestion.companies}
                  onChange={(e) => setEditingQuestion({...editingQuestion, companies: e.target.value})}
                  placeholder="e.g., Google, Microsoft, Amazon"
                />
              </div>
...
interface EditingQuestion extends Omit<TheoryQuestion, 'tags' | 'companies'> {
  tags: string;
  companies: string;
}
