...
import CodeEditor from '@/components/features/questions/CodeEditor';
import { TheoryQuestion as ProblemType, User, Submission, languageTemplates } from '@/lib/mockData';
...
export default function Problem({ problemId, onBack, currentUser }: ProblemPageProps) {
  ...
  const loadProblem = async () => {
    try {
      const data = await MockQuestionAPI.getQuestion(problemId);
      setProblem(data);
      setLoading(false);
  ...
  const handleRun = async () => {
    ...
    try {
      // Mock code execution since MockCodeRunner doesn't exist
      const mockResult = {
        results: [{
          passed: true,
          input: 'test input',
      ...
      setRunResults(mockResult.results);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Mock submission since MockCodeRunner doesn't exist
      const mockSubmission: Submission = {
        ...
        score: 100,
        timestamp: new Date(),
        timeTaken: 60, // 1 minute
        status: 'Accepted' as const,
        language: selectedLanguage,
        runtime: 1000, // 1 second
        memory: 64 // 64 MB
      };
                  ...
                  <Card className="mb-4">
                    <CardContent className="p-4">
                      <div className="space-y-2">
                        <div>
                          <strong>Input:</strong> <code className="bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded">{problem.question}</code>
                        </div>
                        <div>
                          <strong>Output:</strong> <code className="bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded">{problem.correctAnswer}</code>
                        </div>
                        <div>
                ...
                <div>
                  ...
                  <Card className="p-4">
                    <div className="space-y-2 text-sm">
                      <div>• Input size should be reasonable for the given problem</div>
                      <div>• Time complexity should be optimal</div>
                      <div>• Memory usage should be efficient</div>
                      <div>• Handle edge cases appropriately</div>
                    </div>
                  </Card>
                </div>
